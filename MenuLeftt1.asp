    ...
    sub CreateMenu(byval IDLVB)
        ...
        do until rs1.EOF
            ...
            if ul_VB = false then
                ul_VB = true%>
                <ul class='w3-ul menu_left sub_menu'> 
                    <li><span class='ListIcon ListIcon-right'></span> <a href='detail.asp?gr=<%=IDLVB%>&vbpl=<%=rs1("ID")%>'><%=lbl%></a>
            <%else%>
                    </li> <li><span class='ListIcon ListIcon-right'></span> <a href='detail.asp?gr=<%=IDLVB%>&vbpl=<%=rs1("ID")%>'><%=lbl%></a>
            <%end if
